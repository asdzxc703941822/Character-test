  ...
  <div class="about">
    <div class="wrap">
      <mt-radio
        v-if="!second"
        :title="title[num]"
        :options="[item.A1[I_num], item.B1[I_num], item.C1[I_num],item.D1[I_num]]"
        align="right"
        v-model="value"
      ></mt-radio>
      <mt-radio
        v-else-if="second"
        :title="title[num]"
        :options="[item.A2[I_num], item.B2[I_num], item.C2[I_num],item.D2[I_num]]"
        align="right"
        v-model="value"
      ></mt-radio>
      <mt-button @click="next">下一题</mt-button>
    </div>
    <img src="../assets/v2-fcb85aee8d8c923c17023976561177b4_b.jpg" alt>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    next() {
      ...
      if (this.value == "") {
        ...
        this.value = "";
        console.log(this.Anum, this.Bnum, this.Cnum, this.Dnum);
      }
      //
      if (this.num < this.title.length - 1) {
        ...
        this.I_num++;
        //进入后15题要做的操作
        if (this.I_num >= this.item.A1.length) {
          this.I_num = 0;
          this.second = true;
          [
            this.$store.state.Anum,
            this.$store.state.Bnum,
            this.$store.state.Cnum,
            this.$store.state.Dnum
          ] = [this.Anum, this.Bnum, this.Cnum, this.Dnum];
          [
            this.Anum, this.Bnum, this.Cnum, this.Dnum
          ] = [0, 0, 0, 0]; //变量清零
        }
        ...
        [
          this.$store.state.A_1num,
          this.$store.state.B_1num,
          this.$store.state.C_1num,
          this.$store.state.D_1num
        ] = [this.Anum, this.Bnum, this.Cnum, this.Dnum];
  ...
  watch: {
    value() {
      console.log(this.$store.state.Anum);
    }
  }
};
</script>
